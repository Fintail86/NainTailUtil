...
(() => {
  ...
  function render(card, settings) {
    ...
    card.querySelector("[data-output-description]").textContent = hosted
      ? "NainTail 호스트의 CensorTail 출력 위치를 상속합니다. Hosted 모드에서는 변경할 수 없습니다."
      : "검열 결과를 저장할 위치입니다. 기본값은 CensorTail 폴더의 outputs입니다.";
    ...
    card.querySelector("#censorOutputSelect").disabled = settings.locked;
    card.querySelector("#censorOutputReset").disabled = settings.locked || !custom;
  }
  ...
  function createSettingsPage() {
    ...
    page.innerHTML = `
      ...
      <section class="addon-output-settings panel" data-addon-output-settings>
        <div class="addon-output-heading">
          <div><strong>출력 폴더</strong><span data-output-description>검열 결과를 저장할 위치를 관리합니다.</span></div>
        ...
        <div class="addon-output-path-row">
          ...
          <button type="button" id="censorOutputOpen">폴더 열기</button>
        </div>
        <div class="addon-output-actions">
          <button type="button" id="censorOutputReset">기본 위치로 복원</button>
          <button type="button" id="censorOutputSelect" class="primary">위치 변경</button>
        </div>
    ...
    card.querySelector("#censorOutputOpen").addEventListener("click", () => run(card, () => api.openCensorOutputFolder(), "출력 폴더를 열었습니다."));
    card.querySelector("#censorOutputSelect").addEventListener("click", () => run(card, () => api.selectOutputFolder(), "출력 폴더를 변경했습니다."));
    card.querySelector("#censorOutputReset").addEventListener("click", () => run(card, () => api.resetOutputFolder(), "애드온 기본 출력 폴더로 복원했습니다."));
    return page;
  }
  ...
  function install() {
    ...
    if (!settingsTab) {
      ...
      settingsTab.id = "censorSettingsTab";
      settingsTab.className = "tab-item";
      settingsTab.dataset.addonSettingsTab = "";
      settingsTab.innerHTML = '<span class="tab-icon">⚙</span>설정';
