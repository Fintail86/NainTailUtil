...
class HostOutputSettings {
  ...
  setOutputRoot(outputRoot) {
    ...
    if (!normalized || !path.isAbsolute(normalized)) throw new Error("출력 폴더는 절대경로로 지정해야 합니다.");
    const resolved = path.resolve(normalized);
    const parsed = path.parse(resolved);
    if (resolved === parsed.root) throw new Error("드라이브 루트는 출력 폴더로 지정할 수 없습니다.");
...
module.exports = { HostOutputSettings, OUTPUT_SETTINGS_SCHEMA, writeJsonAtomic };
