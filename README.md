...
승인하거나 취소할 수 있다. 호스트에서 실행할 때는 NainTail의 공용 runtime root 안에
AnimaTail과 CensorTail이 함께 쓰는 런타임 한 벌을 사용하고,
...
| 모드 | 런타임과 출력 위치 |
|---|---|
| **Hosted** | NainTail이 관리하는 공용 Python/CUDA 런타임 한 벌과 출력 루트를 사용한다. |
...
NainTail은 공식 애드온의 발견·설치·업데이트·실행과 공용 runtime root·출력 연결을 담당한다.
